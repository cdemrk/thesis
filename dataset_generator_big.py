...
def generate_large_infrastructure():
    """
    Δημιουργεί μια ρεαλιστική, μεγάλης κλίμακας ετερογενή υποδομή (20 Edge + 10 Cloud).
    """
    network = {}
    
    # 1. Δημιουργία 20 Edge Κόμβων
    for i in range(1, 21):
        node_name = f"Edge_{i}"
        network[node_name] = {
...
def generate_workloads_by_capacity(target_gpu_demand):
    ...
    να φτάσει το target_gpu_demand.
    """
    workloads = {}
...
if __name__ == "__main__":
    print("=== Δημιουργία Μεγάλης Υποδομής (20 Edge + 10 Cloud) ===")
    common_network = generate_large_infrastructure()
    ...
    scenarios = [
        {"name": "dataset_1_abundance_big.json", "ratio": 0.50, "desc": "Χωράει Άνετα (50% Demand)"},
        {"name": "dataset_2_marginal_big.json", "ratio": 0.95, "desc": "Χωράει Οριακά (95% Demand)"},
        {"name": "dataset_3_mild_contention_big.json", "ratio": 1.20, "desc": "Δεν Χωράει Οριακά (120% Demand)"},
        {"name": "dataset_4_heavy_contention_big.json", "ratio": 2.50, "desc": "Δεν Χωράει Κατά Πολύ (250% Demand)"}
    ]
    ...
    for scenario in scenarios:
        ...
        print(f"-> Δημιουργήθηκαν {len(workloads)} εργασίες.\n")
