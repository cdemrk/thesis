...
def train_drl_orchestrator(dataset_path, episodes=500, w_cost=0.5, w_idle=0.5, top_k=4, net_type="proposed", save_name="model.pth"):
    print(f"=== Έναρξη Εκπαίδευσης DRL Agent με το {dataset_path} ===")
    print(f"Παράμετροι: W_Cost={w_cost}, W_Idle={w_idle}, Top_K={top_k}, Δίκτυο={net_type.upper()} | Αποθήκευση σε: {save_name}")
    ...
    # Το action_dim είναι πλέον δυναμικό και ίσο με το K
    action_dim = top_k
    
    # Περνάμε το net_type στον Agent για να χτίσει τη σωστή αρχιτεκτονική
    ...
    for episode in range(1, episodes + 1):
        ...
        for step, (w_id, current_workload) in enumerate(workload_items):
            ...
            # Κόβουμε τη λίστα στο K (αντί για 10)
...
if __name__ == "__main__":
    # Τρέχουμε την εκπαίδευση για το Proposed δίκτυο
    trained_agent = train_drl_orchestrator(
        dataset_path="dataset_3_mild_contention.json", 
        episodes=1500,
        w_cost=0.5,
        w_idle=0.5,
        top_k=4,
        net_type="deep", # <--- Η αρχιτεκτονική
        save_name="weights_comparison_deep_top4-0.5-0.5.pth" # <--- Το όνομα που ψάχνει το eval_model.py
    )